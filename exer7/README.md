...
First, setup a cluster if you already do not have one in your MongoDB Atlas Cluster, and get the connection URL. Or setup a database locally (like I did in lecture)
...
The function to connect to the database is in `server/mongodb/index.ts`.
...
All API endpoints should exist in the `app/api` folder of your Next.js project. This means all api routes will be prefixed with `/api` (helpful for testing). Note: The names we gave for these API ednpoints are not conventional, we just separated them to make it easier for learning.
...
- Fill in all the endpoints in `app/api`
